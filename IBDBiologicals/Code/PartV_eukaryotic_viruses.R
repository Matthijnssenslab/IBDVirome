...
####################################
## Take into account that most of diversity analysis already corrects for sequencing depth (alpha-diversity), so you have to feed it the unrarefied completeness data
...
####################################
# 2.1 Mastertable: evaluate (~ use for figures)
####################################
...
####################################
# 2.2 Calculate AS (AF*AAI/ANI) (if you did this before skip it)
####################################
# 2.2.1 Unrarefied
####################################
# Add AS
...
####################################
# 4.1 stacked barplot: absolute number of reads
####################################
...
ggplot(Mastertable_viral_rarefied_categories_euk_gen, aes(x=sample, y= reads)) +
  ...
  theme(axis.text.y = element_text(color = "black"))
####################################
# 4.2 Heatmap relative abundances
####################################
# 4.2.1 All samples
####################################
# (A) aggregate 
...
ggplot(Mastertable_viral_rarefied_categories_euk_gen_normalized, aes(x=sample, y= reads)) +
  ...
  theme(axis.text.x = element_text(size = 5, angle = 45, vjust = 1 , hjust = 1, color = "black"), axis.title = element_text(size = 9))
####################################
# 4.2.2 Samples with eukaryotic virus presence
####################################
