...
# Rarefaction is method used in which we will rarefy to the sample ad random with the lowest number of reads. because, we throw away valuable data we have to find a balance between (i) discarding of precious data and (ii) the samples that have stil lreads left after discarding this data. For example, it might be that if we rarefy to 100.000 reads we include all of are samples, but discard 99% of the data. Instead of we rarefy to 1 million reads we keep 90% of the data but discard a few samples.
# Rarefaction is performed to normalize for sequencing depth in which some sample will be deeper sequenced (more reads) than others (less reads). If rarefaction is performed the data is pressumed to be useable for a quantitative comparison.
# Rarefaction can be performed on (i) the total number of reads, or on (ii) the viral number of reads. Either choice will have its own advantages and disadvantages.

# My preference goes to rarefaction based on the viral number of reads. I make the assumption here that if samples are sequenced deeper also their viral subset will be sequenced deeper.
# In the past I compared the two methods and did not find any significant difference between the one or the other, therefore preferences dominates in this decision.
# After viral rarefaction, the viral read data is equal between also samples making the quantitative comparisons between samples more correct and able to compare viromes between samples.
# if possible I would like to take a standard cutoff of 1M reads, so in the future data will be comparable too using this same threshold. Easy number to hold too.

# PS: For diversity analysis, we still have the fragmentation problem. For that reason we need to correct viral contigs based on completeness and keep all viral contigs that are > 50% completete ccording to CheckV.
# PS2: This rarefied completeness viral data can be used for diversity analysis.
# PS3: Tjis script is more manual
####################################
